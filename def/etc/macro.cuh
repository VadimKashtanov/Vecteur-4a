...
//
#define PLUME_CMP(a, b) do {float mul = (a/b);if (sng(a)!=sng(b)) {printf("\033[4m");}; if (mul <= BORNE_HAUT_VERT && mul >= BORNE_BAS__VERT) {printf("\033[92m");} else if (mul <= BORNE_HAUT_JAUNE && mul >= BORNE_BAS__JAUNE) {printf("\033[93m");} else {printf("\033[91m");};printf("%+f === %+f \033[0m", a, b);} while(0);
